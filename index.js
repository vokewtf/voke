// Developer/Owner: Zylux | zylux#3700 On Discord | Zylux with 130 Subs On YT | 
// Open Terminal and type "npm i" once done, Type "node ." in the terminal/shell and the bot will log-on.
// NzQ3NTYwNDc1OTE0NDAzODkw.X0Qp8g.bgonQl3V56004qcdK8flri5hz94
...
bot.user.setPresence({ activity: { name: 'vc!help | Vokes Community' } , status: 'idle' }) 
})
...
fs.readdir('./player-events/', (err, files) => {
    ...
    files.forEach(file => {
        ...
        console.log(`Launching ${eventName} Was Successful! - Zylux`);
...
bot.on("message", async message => {
    ...
    if (command === 'help') {
        const helpEmbed = new MessageEmbed()
            ...
            .addField("Fun", "`say` `gay` `calc` `covid` `meme` `dog` `cat` `ascii` `docs` `roast`")
